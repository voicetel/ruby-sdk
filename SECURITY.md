...
<https://github.com/voicetel/ruby-sdk/security/advisories/new>).
...
- The SDK runs Faraday on the `net_http_persistent` adapter to keep
  TCP + TLS sessions alive across requests at TLS 1.2+. If you supply
  a custom Faraday adapter, you are responsible for matching that
  posture.
...
Out of scope: vulnerabilities in the `voicetel` RubyGem caused by a
